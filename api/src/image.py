...
class ImageGenerator:
    def on_generate_image_done(self, task, image_uid):
        try:
            ...
            logger.error(f'{BOLD}{image_uid}{RESET} {RED}Generate image FAIL{RESET} ...')
    ...
    async def generate_image_and_save(self, image_uid: str, prompt: str):
        try:
            image_bytes = await self.generate_image(image_uid, prompt)
            ...
            logger.error(f'{BOLD}{image_uid}{RESET} {RED}Generate image FAIL{RESET} ... elapsed {BOLD}{time.time() - start_time}{RESET}s')

    async def generate_image(self, image_uid: str, prompt: str):
        payload = {
            prompt: prompt,
            scales: 3.5,
            steps: 20,
