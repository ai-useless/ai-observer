...
type SearchResponseType = SearchResponsePayload | ErrorResponsePayload
...
export class SearchWorker {
  ...
  private _listeners: Map<SearchEventType, ListenerFunc[]> = new Map<
    SearchEventType,
    ListenerFunc[]
  >()
  ...
  private constructor() {
    ...
    this._worker.onMessage((payload: Taro.Worker.OnMessageCallbackResult) => {
      const event = payload.message as SearchEvent
