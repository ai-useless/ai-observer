...
type SearchResponseType = GenerateRequestPayload | ErrorResponsePayload
...
export class SearchWorker {
  ...
  private _listeners: Map<DuanziEventType, ListenerFunc[]> = new Map<
    DuanziEventType,
    ListenerFunc[]
  >()

  private constructor() {
    this._worker = Taro.createWorker('worker/worker/duanzi/worker.js')

    this._worker.onMessage((payload: Taro.Worker.OnMessageCallbackResult) => {
      const event = payload.message as DuanziEvent
  ...
  public static on = (type: DuanziEventType, listener: ListenerFunc) => {
  ...
  public static off = (type: DuanziEventType, listener: ListenerFunc) => {
    const listeners = SearchWorker.getSearchWorker()._listeners.get(type) || []
