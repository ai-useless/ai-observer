...
export class EChat {
  static chat = (simulatorId: number, simulator: string, personality: string, myName: string, messages: string[], modelId: number, onMessage: (message?: string, audio?: string, error?: string) => void) => {
    chatWorker.ChatRunner.handleGenerateRequest({
      ...
      if (!payload || !payload.text || !payload.text.length) {
        onMessage(undefined, undefined, '你的伙伴好像什么也没有发送给你！这是传说中的空包吗？')
        return
      }
