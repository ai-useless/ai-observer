...
export const Prompts = new Map<Intent, IntentFunc>([
  ...
  [Intent.START_TOPIC, ((personality: string, topicMaterial: string, speakDuration: number) => `你的人设是${personality}，现在是节目的开始，本期节目的主要内容为：${topicMaterial}，
                                       请你就主要内容做一个简单的开场，需达成：1) 发言时间≤${speakDuration}秒,小于100字 2) 只输出发言内容，不允许有表情，标签，换行符和提示`) as IntentFunc],
  [Intent.START_SUBTOPIC, ((topic: string, personality: string, topicMaterial: string, subTopic: string, speakDuration: number) => `你的人设是${personality}，今天讨论的主题为："${topic}",
                                       ...
                                       1)直接输出发言内容，不允许有表情，标签，换行符和提示 2) 发言时间≤${speakDuration}秒，小于100字`) as IntentFunc],
  ...
  [Intent.CONCLUDE, ((personality: string, topicMaterial: string, speakDuration: number) => `你的人设是${personality}，现在是节目的最后，本期节目的主要内容为：${topicMaterial}，
                                       请你对本期内容进行总结陈词，需达成：1) 发言时间≤${speakDuration}秒,小于100字 2) 只输出发言内容，不允许有表情，标签，换行符和提示`) as IntentFunc]
])
